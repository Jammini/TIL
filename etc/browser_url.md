...
우리는 매일 웹 브라우저를 열고 즐겨찾는 웹사이트를 탐색한다. 주소창에 URL을 입력하거나 페이지 링크를 클릭하면 해당 페이지로 이동한다. 그런데, 그 뒷단에서 무슨 일이 일어나는지 생각해 본 적이 있는가?
...
웹 브라우저에서 URL을 입력했을때, 일어나는 일들을 살펴보았다. 간단하게 정리하자면, 아래와 같다.
